...
for m < 5 { // Das ist doch auch eine schleife oder ?
	// Ja, das ist eine andere Art von Schleife in Go, die als "while"-Schleife bezeichnet wird. In diesem Fall wird die Schleife so lange ausgeführt, wie die Bedingung "m < 5" wahr ist. Innerhalb der Schleife kannst du den Wert von "m" ändern, um sicherzustellen, dass die Schleife irgendwann endet.
...
arch := uint16(0x3E) // Das ist eine Variable vom Typ uint16, die den Wert 0x3E (62 in Dezimal) enthält. In diesem Fall könnte es sich um eine Architekturkennung handeln, die in einem ELF-Header verwendet wird.
...
default: // Wenn "arch" keinen der vorherigen Werte hat, wird dieser Fall ausgeführt.
	fmt.Printf("Unknown architecture: 0x%X\n", arch) // In diesem Fall wird die Ausgabe "Unknown architecture: 0x3E" angezeigt, wobei der Wert von "arch" in Hexadezimalform dargestellt wird.
}		